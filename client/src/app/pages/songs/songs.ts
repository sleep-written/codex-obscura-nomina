...
import { SongImport } from '../../shared/song/song-import';
...
export class Songs {
  ...
  protected onNew(): void {
    if (!this.songImport.confirmDiscard()) return;
    this.store.startNew();
  ...
  protected onOpen(id: string): void {
    ...
    if (this.store.currentId() !== id && !this.songImport.confirmDiscard()) return;
  ...
  protected async onImport(event: Event): Promise<void> {
    ...
    await this.songImport.fromLyricsText(await this.fileIo.readTextFile(file));
  }
}
