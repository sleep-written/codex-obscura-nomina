...
| `#` | Título de estrofa (opcional, igual que en Markdown). |
| `\n` | Fin de verso. |
...
**How to apply:** cualquier trabajo en `./lyrics-language` (tokenizer, parser, analizador métrico) debe respetar esta tabla de símbolos tal cual. Ver [[lyrics-tokenizer]] para la implementación del tokenizer que consume este DSL, [[lyrics-app-architecture]] para el porqué del marcado obligatorio, y [[local-memory-convention]] para el flujo de `memory-draft`.
